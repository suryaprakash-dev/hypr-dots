...
export thmbDir
export dcolDir
export hydewalldir="${hydeConfDir}/wallpapers"

# Use wallpaper directory instead of theme directory
[ -d "${hydewalldir}" ] && cacheIn="${hydewalldir}" || exit 1
...
while getopts "w:d:f" option ; do
    ...
    w ) # generate cache for input wallpaper
        ...
        cacheIn="${OPTARG}"
        ;;
    d ) # generate cache for specific wallpaper directory
        cacheIn="${OPTARG}"
        if [ ! -d "${cacheIn}" ] ; then
        ...
        ;;
    f ) # full cache rebuild for wallpaper directory
        cacheIn="${hydewalldir}"
        mode="_force"
        ;;
    * ) # invalid option
        ...
        echo "d : generate cache for specific wallpaper directory"
        echo "f : full cache rebuild for wallpaper directory"
        exit 1 ;;
    esac
done
