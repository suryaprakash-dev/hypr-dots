...
# Load wallpaper colors (always use wallpaper colors, no theme override)
set -a
source "${wallbashOut}"
# Force dark mode
dcol_mode="dark"
...
#// deploy wallbash colors function (simplified for dark mode only)
fn_wallbash () {
    local tplt="${1}"
    ...
    # Extract target path and optional command from first line
    eval target="$(head -1 "${tplt}" | awk -F '|' '{print $1}')"
    [ ! -d "$(dirname "${target}")" ] && echo "[skip: no dir] \"${target}\"" && return 0
    appexe="$(head -1 "${tplt}" | awk -F '|' '{print $2}')"
    
    # Copy template content (skip first line) to target
    sed '1d' "${tplt}" > "${target}"
    
    # Replace color placeholders with dark mode colors (no inversion)
...
# Disable Hyprland auto-reload during processing if running in Hyprland
[ -n "$HYPRLAND_INSTANCE_SIGNATURE" ] && hyprctl keyword misc:disable_autoreload 1 -q && trap 'hyprctl reload -q && echo "[wallbash] reload :: Hyprland"' EXIT

# Run wallbash icon script
$HOME/.config/hypr/scripts/wallbashicon.sh

# Deploy wallbash colors - only process .dcol files
echo ":: deploying wallbash colors :: dark mode"
