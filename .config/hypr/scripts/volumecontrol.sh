...
notify_vol() {
    ...
    notify-send -a "t2" -r 91190 -t 800 -i "${ico}" ``"volume" "${vol}${bar}"
}
...
notify_mute() {
    ...
    if [ "${mute}" == "true" ]; then
        notify-send -a "t2" -r 91190 -t 800 -i "${icodir}/muted-${dvce}.svg" "muted" "${dvce} Turned OFF"
    else
        notify-send -a "t2" -r 91190 -t 800 -i "${icodir}/unmuted-${dvce}.svg" "unmuted" "${dvce} Turned ON"
    fi
}
